...
// Takes care of output.  Returns true on success.
template <typename FST>
bool DecodeUtteranceLatticeFasterMapVersion(
    LatticeFasterMapDecoderTpl<FST> &decoder, // not const but is really an input.
    DecodableInterface &decodable, // not const but is really an input.
    const TransitionModel &trans_model,
    const fst::SymbolTable *word_syms,
    std::string utt,
...
// Instantiate the template above for the two required FST types.
template bool DecodeUtteranceLatticeFasterMapVersion(
    LatticeFasterMapDecoderTpl<fst::Fst<fst::StdArc> > &decoder,
    DecodableInterface &decodable,
    const TransitionModel &trans_model,
    const fst::SymbolTable *word_syms,
    std::string utt,
...
template bool DecodeUtteranceLatticeFasterMapVersion(
    LatticeFasterMapDecoderTpl<fst::GrammarFst> &decoder,
    DecodableInterface &decodable,
    const TransitionModel &trans_model,
    const fst::SymbolTable *word_syms,
    std::string utt,
