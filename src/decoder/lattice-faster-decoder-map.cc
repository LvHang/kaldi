// decoder/lattice-faster-decoder-map.cc
...
#include "decoder/lattice-faster-decoder-map.h"
...
LatticeFasterMapDecoderTpl<FST, Token>::LatticeFasterMapDecoderTpl(
    const FST &fst,
    const LatticeFasterDecoderConfig &config):
    fst_(&fst), delete_fst_(false), config_(config), num_toks_(0) {
  config.Check();
  cur_toks_.reserve(1000);  // just so on the first frame we do something reasonable.
}
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::InitDecoding() {
...
template <typename FST, typename Token>
bool LatticeFasterMapDecoderTpl<FST, Token>::Decode(DecodableInterface *decodable) {
  InitDecoding();
...
template <typename FST, typename Token>
bool LatticeFasterMapDecoderTpl<FST, Token>::GetBestPath(Lattice *olat,
...
template <typename FST, typename Token>
bool LatticeFasterMapDecoderTpl<FST, Token>::GetRawLattice(
    Lattice *ofst,
    bool use_final_probs) const {
...
template <typename FST, typename Token>
bool LatticeFasterMapDecoderTpl<FST, Token>::GetLattice(CompactLattice *ofst,
                                             bool use_final_probs) const {
  Lattice raw_fst;
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::PossiblyResizeHash(size_t num_toks) {
  size_t new_sz = static_cast<size_t>(static_cast<BaseFloat>(num_toks)
                                      * config_.hash_ratio);
  if (new_sz > cur_toks_.size()) {
    cur_toks_.reserve(new_sz);
  }
}
...
// FindOrAddToken either locates a token in hash of cur_toks_,
// or if necessary inserts a new, empty token (i.e. with no forward links)
// for the current frame.  [note: it's inserted if necessary into hash cur_toks_
// and also into the singly linked list of tokens active on this frame
// (whose head is at active_toks_[frame]).
template <typename FST, typename Token>
Token* LatticeFasterMapDecoderTpl<FST, Token>::FindOrAddToken(
  ...
  typename StateIdToTokenMap::iterator e_found = cur_toks_.find(state);
  if (e_found == cur_toks_.end()) {  // no such token presently.
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::PruneForwardLinks(
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::PruneForwardLinksFinal() {
...
template <typename FST, typename Token>
BaseFloat LatticeFasterMapDecoderTpl<FST, Token>::FinalRelativeCost() const {
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::PruneTokensForFrame(int32 frame_plus_one) {
  KALDI_ASSERT(frame_plus_one >= 0 && frame_plus_one < active_toks_.size());
  Token *&toks = active_toks_[frame_plus_one].toks;
  if (toks == NULL)
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::PruneActiveTokens(BaseFloat delta) {
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::ComputeFinalCosts(
  ...
  for (typename StateIdToTokenMap::const_iterator iter = cur_toks_.begin();
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::AdvanceDecoding(DecodableInterface *decodable,
  ...
  if (std::is_same<FST, fst::Fst<fst::StdArc> >::value) {
    ...
    if (fst_->Type() == "const") {
      LatticeFasterMapDecoderTpl<fst::ConstFst<fst::StdArc>, Token> *this_cast =
          reinterpret_cast<LatticeFasterMapDecoderTpl<fst::ConstFst<fst::StdArc>, Token>* >(this);
      this_cast->AdvanceDecoding(decodable, max_num_frames);
      return;
    } else if (fst_->Type() == "vector") {
      LatticeFasterMapDecoderTpl<fst::VectorFst<fst::StdArc>, Token> *this_cast =
          reinterpret_cast<LatticeFasterMapDecoderTpl<fst::VectorFst<fst::StdArc>, Token>* >(this);
      this_cast->AdvanceDecoding(decodable, max_num_frames);
      return;
    }
  }
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::FinalizeDecoding() {
...
template <typename FST, typename Token>
BaseFloat LatticeFasterMapDecoderTpl<FST, Token>::GetCutoff(
    StateIdToTokenMap *map, size_t *tok_count,
    BaseFloat *adaptive_beam, StateId *best_state, Token* *best_tok) {
  BaseFloat best_weight = std::numeric_limits<BaseFloat>::infinity();
  // positive == high cost == bad.
  size_t count = 0;
  if (config_.max_active == std::numeric_limits<int32>::max() &&
      config_.min_active == 0) {
    for (typename StateIdToTokenMap::iterator e = map->begin();
         e != map->end(); e++, count++) {
      BaseFloat w = static_cast<BaseFloat>(e->second->tot_cost);
      if (w < best_weight) {
        best_weight = w;
        if (best_tok) {
          *best_state = e->first;
...
template <typename FST, typename Token>
BaseFloat LatticeFasterMapDecoderTpl<FST, Token>::ProcessEmitting(
  ...
  prev_toks_.swap(cur_toks_);  // analogous to swapping prev_toks_ / cur_toks_
  cur_toks_.clear();           // in simple-decoder.h.   Removes the Elems from
  ...
  size_t tok_cnt;
  BaseFloat cur_cutoff = GetCutoff(&prev_toks_, &tok_cnt, &adaptive_beam,
                                   &best_state, &best_tok);
  ...
  // the tokens are now owned here, in prev_toks_.
  // 'owned' is a complex thing here; the point is we need to call DeleteElem
  // on each token 'e' to let prev_toks_ know we're done with them.
  for (typename StateIdToTokenMap::iterator e = prev_toks_.begin();
       e != prev_toks_.end(); e++) {
    // loop this way because we delete "e" as we go.
    StateId state = e->first;
    Token *tok = e->second;
    if (tok->tot_cost <= cur_cutoff) {
      for (fst::ArcIterator<FST> aiter(*fst_, state);
        ...
        if (arc.ilabel != 0) {  // propagate..
          ...
          // hence the + 1.
          Token *tok_next = FindOrAddToken(arc.nextstate,
                                           frame + 1, tot_cost, tok, NULL);
          // NULL: no change indicator needed

          // Add ForwardLink from tok to next_tok (put on head of list tok->links)
          tok->links = new ForwardLinkT(tok_next, arc.ilabel, arc.olabel,
...
// static inline
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::DeleteForwardLinks(Token *tok) {
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::ProcessNonemitting(BaseFloat cutoff) {
  KALDI_ASSERT(!active_toks_.empty());
  ...
  for (typename StateIdToTokenMap::iterator iter = cur_toks_.begin();
       iter != cur_toks_.end();  iter++) {
    StateId state = iter->first;
  ...
  while (!queue_.empty()) {
    ...
    queue_.pop_back();
    Token *tok = cur_toks_[state];  // would segfault if e is a NULL pointer
    ...
    for (fst::ArcIterator<FST> aiter(*fst_, state);
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        ...
        if (tot_cost < cutoff) {
          ...
          Token *tok_new = FindOrAddToken(arc.nextstate, frame + 1, tot_cost,
                                       tok, &changed);

          tok->links = new ForwardLinkT(tok_new, 0, arc.olabel,
...
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::ClearActiveTokens() {
...
// static
template <typename FST, typename Token>
void LatticeFasterMapDecoderTpl<FST, Token>::TopSortTokens(
...
// that we'll need.
template class LatticeFasterMapDecoderTpl<fst::Fst<fst::StdArc>, decoder::StdToken>;
template class LatticeFasterMapDecoderTpl<fst::VectorFst<fst::StdArc>, decoder::StdToken >;
template class LatticeFasterMapDecoderTpl<fst::ConstFst<fst::StdArc>, decoder::StdToken >;
template class LatticeFasterMapDecoderTpl<fst::GrammarFst, decoder::StdToken>;

template class LatticeFasterMapDecoderTpl<fst::Fst<fst::StdArc> , decoder::BackpointerToken>;
template class LatticeFasterMapDecoderTpl<fst::VectorFst<fst::StdArc>, decoder::BackpointerToken >;
template class LatticeFasterMapDecoderTpl<fst::ConstFst<fst::StdArc>, decoder::BackpointerToken >;
template class LatticeFasterMapDecoderTpl<fst::GrammarFst, decoder::BackpointerToken>;
