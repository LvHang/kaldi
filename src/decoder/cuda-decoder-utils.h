...
// This class is used in gpu-decoding stuff. In this stuff, the BatchComputer
// generate the log-likelihoods chunk by chunk. This class is just used to pass
// the address.
class CuMatrixChunker {
 public:
  // This constructor creates an object that will not delete "likes"
  // when done. So the CuMatrixChunker gets one chunk each time.
  CuMatrixChunker(const CuMatrix<BaseFloat> &likes, int chunk_len): likes_(&likes),
    delete_likes_(false), chunk_len_(chunk_len) {
  }
  ...
  void LogLikelihoodChunk(int32 frame, CuMatrixBase<BaseFloat>** out, cudaStream_t stream) {
    if (frame >= likes_->NumRows()) return;
    int len = std::min(chunk_len_, likes_->NumRows() - frame);
    assert(len);
    loglikes_ = new CuSubMatrix<BaseFloat>(likes_->RowRange(frame, len));
    *out = loglikes_;
    return;
  };
  ...
  bool delete_likes_;
  CuMatrixBase<BaseFloat>* loglikes_;
