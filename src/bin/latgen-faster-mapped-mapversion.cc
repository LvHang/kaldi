// bin/latgen-faster-mapped-mapversion.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: latgen-faster-mapped-mapversion [options] trans-model-in (fst-in|fsts-rspecifier) loglikes-rspecifier"
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
      ...
      {
        LatticeFasterMapDecoder decoder(*decode_fst, config);
        ...
        for (; !loglike_reader.Done(); loglike_reader.Next()) {
          ...
          double like;
          if (DecodeUtteranceLatticeFasterMapVersion(
      ...
      for (; !fst_reader.Done(); fst_reader.Next()) {
        ...
        LatticeFasterMapDecoder decoder(fst_reader.Value(), config);
        DecodableMatrixScaledMapped decodable(trans_model, loglikes, acoustic_scale);
        double like;
        if (DecodeUtteranceLatticeFasterMapVersion(
