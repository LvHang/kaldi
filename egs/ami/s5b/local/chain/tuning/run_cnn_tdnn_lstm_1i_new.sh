...
if [ $stage -le 15 ]; then
  ...
  relu-renorm-layer name=ivector-1 input=ivector dim=128
  relu-renorm-layer name=ivector-2 dim=128
  ...
  relu-renorm-layer name=tdnn6 dim=256 input=Append(-3,0,3,ReplaceIndex(ivector-2, t, 0))
  relu-renorm-layer name=tdnn7 dim=256 input=Append(-6,-3,0)
  ...
  relu-renorm-layer name=prefinal-xent input=tdnn7 dim=256 target-rms=0.5
