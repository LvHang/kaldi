...
if [ $stage -le 3 ]; then
  #dump egs                                         
  if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d $fvector_dir/egs/storage ]; then  
    utils/create_split_dir.pl \                                                 
    /export/b{03,11,12,13}/$USER/kaldi-data/egs/minilibrispeech-$(date +'%m_%d_%H_%M')/s5/$egs_dir/storage $egs_dir/storage
  fi                                                                            
