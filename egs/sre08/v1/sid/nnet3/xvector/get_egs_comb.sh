...
mode=mod2                   # choices=[mod1, mod2, mod3], for details, please
                            # please see allocate_egs_seg_comb.py
#used in mode1
num_repeats=1
...
# prepare the special options for different modes
special_opts=
if [ $mode == "mod1" ]; then
  [ ! -z $num_repeats ] && special_opts="--num-repeats=$num_repeats"
elif [ $mode == "mod2" ]; then
  [ ! -z $frames_per_kind ] && special_opts="--frames-per-kind=$frames_per_kind"
elif [ $mode == "mod3" ]; then
  [ ! -z num_egs_per_speaker ]  && \
    $num_egs_per_speaker_per_length=$[$num_egs_per_speaker/$kinds_of_length_train]
  special_opts="--num_egs_per_speaker_per_length=$num_egs_per_speaker_per_length"
else
  echo "$0: Select a mode at first" && exit 0
fi 
...
if [ $stage -le 2 ];then
  ...
  if [ $mode == "mod1" ]; then
    ...
    total_frames=$[$num_train_set_frames*$num_repeats]
    num_train_archives=$[$total_frames/$frames_per_iter]
    ...
    num_train_archives=$[$frames_per_kind*$kinds_of_length_train/$frames_per_iter]
    feat_scps=$(for n in `seq $num_train_archives`; do echo $data_combine/shuffled_feats.${n}.scp; done)
    utils/split_scp.pl ${data_combine}/shuffled_feats.scp $feat_scps
  elif [ $mode == "mod3" ]; then
